...
local function DoDuplicateStuff()
  ...
  for i = 1, dupenum do
    ...
    if not otherLanes then reaper.Main_OnCommand(42691, 0) end --Track lanes: Delete lanes (including media items) that are not playing
    ...
    -- TODO: ENVELOPES
    if not envelopes then reaper.Main_OnCommand(reaper.NamedCommandLookup("_S&M_REMOVE_ALLENVS"), 0) end
